...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (method === 'GET') {
      // Obtener documentos de BSale
      const response = await fetch(`${bsaleUrl}/documents.json?limit=50&documenttypeid=1`, {
        headers: {
      ...
      const response = await fetch(`${bsaleUrl}/documents.json`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
    ...
    console.error('Error en proxy BSale documentos:', error);
    res.status(500).json({ 
